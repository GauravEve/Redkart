...
export function loadProducts(currentPage){
    return async (dispatch)=>{
        return await axios.get(`http://localhost:5000/getProducts?page=${currentPage}`).then((response)=>{
...
export function loadCarts(){
    return async (dispatch)=>{
        return await axios.get("http://localhost:5000/getCart").then((response)=>{
            // console.log(response);
...
export function loadCartSaves(){
    return async (dispatch)=>{
        return await axios.get("http://localhost:5000/getCartSave").then((response)=>{
            // console.log(response);
...
export function cartHandler(ele){
    return async ()=>{
    console.log(ele);
        return await axios.post('http://localhost:5000/cartHandler',{id:ele.id,sign:ele.sign})
        .then((res)=>{
...
export function cartSave(ele){
    return async ()=>{
    console.log(ele);
        return await axios.post('http://localhost:5000/cartSave',ele)
        .then((res)=>{
        console.log('Item Incremented');
        res.status(201).send({message:"Added successfully"})
...
export function addToCart(ele){
    return async ()=>{
    console.log(ele);
        return await axios.post('http://localhost:5000/cart',ele)
        .then((res)=>{
